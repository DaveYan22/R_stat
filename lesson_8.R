...
ggplot()+geom_density(data=data, mapping=aes(x=price,fill = fuelType), alpha = 0.3)

ggplot()+geom_histogram(data=data, mapping=aes(x=price,fill = fuelType), alpha = 0.3)
...
ggplot(FirstData, aes(y=price, x=mileage, color=transmission)) + 
  geom_point()


model11 <- lm(price ~ mileage, FirstData)
summary(model11)
...
# year -> age -2020 then abs()
# engineSize -> categorical

summary(data$year)
table(data$engineSize)

data$age <- abs(data$year-2020)
data$mileage_scaled <- data$mileage
data$log_price <- log(data$price)
data_outlier <- data[data$price<60000 & data$price>6500 & data$engineSize>0 & data$mpg<80, ]
quantile(data$price,0.01)
quantile(data$price,0.99)

ggplot()+geom_boxplot(data = data, mapping = aes(x=transmission, y=price))
ggplot()+geom_boxplot(data = data, mapping = aes(x=fuelType, y=price))
ggplot()+geom_point(data = data, mapping = aes(x=mileage, y=log(price)))
ggplot()+geom_boxplot(data = data, mapping = aes(x=as.factor(tax), y=price))
ggplot()+geom_point(data = data, mapping = aes(x=engineSize, y=price))+
  geom_smooth(data = data, mapping = aes(x=engineSize, y=price), method = "lm")
ggplot()+geom_point(data = data, mapping = aes(x=mpg, y=log(price)))
ggplot()+geom_point(data = data, mapping = aes(x=age, y=log(price)))


model_fin <- lm(log_price~transmission+fuelType+mileage+engineSize+mpg+age, 
                data = data)
summary(model_fin)

cor.test(data$mpg, data$mileage)
